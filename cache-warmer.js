...
class CacheWarmer {
  ...
  async warmCache(force = false) {
    ...
    try {
      ...
      Logger.info("CacheWarmer", `Cache warming concluído em ${totalTime.toFixed(2)}ms`, warmingResult);
  ...
  async getUsagePatterns() {
    try {
      ...
      Logger.debug("CacheWarmer", `Padrões de uso carregados: ${Object.keys(patterns).length} módulos`);
      return patterns;
    } catch (error) {
  ...
  async recordModuleUsage(moduleName) {
    try {
      ...
      Logger.debug("CacheWarmer", `Uso registrado para ${moduleName}: ${modulePattern.totalUses} usos`);
  ...
  calculatePriorityModules(patterns) {
    ...
    for (const module of availableModules) {
      ...
      if (pattern) {
        ...
        const daysSinceLastUse = (now - pattern.lastUsed) / (24 * 60 * 60 * 1000);
        const recencyScore = Math.max(30 - daysSinceLastUse * 5, 0);
        
        // Fator de consistência (0-20 pontos)
        const consistencyScore = pattern.averageInterval > 0 ? 
          Math.min(20 / (daysSinceLastUse + 1), 20) : 0;
        
        // Fator de uso total (0-10 pontos)
        const totalUsesScore = Math.min(pattern.totalUses, 10);
        
        score = frequencyScore + recencyScore + consistencyScore + totalUsesScore;
      } else {
    ...
    const sortedModules = Array.from(scores.entries())
      ...
      .slice(0, 5)
      .map(([module, score]) => {
        Logger.debug("CacheWarmer", `Módulo ${module} score: ${score.toFixed(1)}`);
        return module;
      });
    
    Logger.info("CacheWarmer", `Módulos prioritários calculados: ${sortedModules.join(', ')}`);
  ...
  async preloadModules(modules) {
    ...
    Logger.info("CacheWarmer", `Pré-carregando ${modules.length} módulos prioritários...`);
      ...
      const batchPromises = batch.map(async (module) => {
        ...
        try {
          ...
          if (moduleLoader.isModuleLoaded(module)) {
            Logger.debug("CacheWarmer", `Módulo ${module} já carregado, pulando...`);
            return {
              module,
              success: true,
              cached: true,
          ...
          Logger.debug("CacheWarmer", `Módulo ${module} pré-carregado em ${loadTime.toFixed(2)}ms`);
          ...
          const loadTime = performance.now() - moduleStartTime;
          Logger.warn("CacheWarmer", `Falha no pré-carregamento de ${module}:`, error);
    ...
    results.averageLoadTime = results.loadTimes.length > 0 ? 
      results.loadTimes.reduce((a, b) => a + b, 0) / results.loadTimes.length : 0;
    results.maxLoadTime = results.loadTimes.length > 0 ? Math.max(...results.loadTimes) : 0;
  ...
  async recordWarmingMetrics(result, totalTime) {
    try {
      ...
      metrics.successRate = metrics.totalModulesWarmed / (metrics.totalWarmings * 5); // Assumindo 5 módulos por warming
  ...
  async getWarmingMetrics() {
    try {
      const stored = await browser.storage.local.get([this.warmingMetricsKey]);
      return stored[this.warmingMetricsKey] || {
        totalWarmings: 0,
        totalModulesWarmed: 0,
        totalTime: 0,
        averageTime: 0,
  ...
  async cleanupOldPatterns(maxAge = 30 * 24 * 60 * 60 * 1000) {
    try {
      ...
      if (removedCount > 0) {
        ...
        Logger.info("CacheWarmer", `Removidos ${removedCount} padrões de uso antigos`);
      }
  ...
  async getStatus() {
    ...
    return {
      ...
      cooldownRemaining: Math.max(0, this.warmingCooldown - (Date.now() - this.lastWarmingTime)),
