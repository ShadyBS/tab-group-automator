      ...
      if (tab.title) {
        ...
        patterns.forEach((pattern) => {
          analysis.patterns.set(
            pattern,
            (analysis.patterns.get(pattern) || 0) + 1
          );
        });
      }
    ...
    for (const pattern of patterns) {
      try {
        ...
        switch (pattern.type) {
          ...
          case 'exact':
            isMatch =
              (item.text || item.title || item.url || '') === pattern.value;
            break;
...
async function optimizeCache(data) {
  ...
  switch (strategy) {
    case 'lru':
      sortedEntries = cacheEntries.sort(
  ...
  for (const entry of sortedEntries) {
    if (
      optimized.length < maxSize &&
      currentSize + entry.size <= maxSize * 1.2
    ) {
      optimized.push({
        key: entry.key,
        size: entry.size,
...
function categorizeTab(tab) {
  ...
  if (url.includes('docs.google.com') || url.includes('office.com'))
    return 'documents';
...
function generateRegexSummary(results) {
  ...
  summary.averageMatchesPerText =
    results.length > 0 ? summary.totalMatches / results.length : 0;
