...
/**
 * Classe wrapper para APIs do navegador com rate limiting
 */
class BrowserAPIWrapper {
  constructor() {
    ...
    Logger.debug("BrowserAPIWrapper", `Rate limiting ${this.rateLimitingEnabled ? 'habilitado' : 'desabilitado'}`);
  }
  
  /**
   * Cria proxy para interceptar chamadas de API
   * @returns {Proxy} Proxy das APIs do navegador
   */
  createAPIProxy() {
    const self = this;
  ...
  /**
   * Cria proxy para tabs API
   * @returns {Proxy} Proxy da tabs API
   */
  createTabsProxy() {
    const self = this;
    ...
    return new Proxy(browser.tabs, {
      get(target, prop) {
        switch (prop) {
          case 'get':
            return (tabId, context = {}) => self.executeWithRateLimit(
          ...
          case 'query':
            return (queryInfo, context = {}) => self.executeWithRateLimit(
          ...
          case 'group':
            return (options, context = {}) => self.executeWithRateLimit(
          ...
          case 'ungroup':
            return (tabIds, context = {}) => self.executeWithRateLimit(
  ...
  /**
   * Cria proxy para tabGroups API
   * @returns {Proxy} Proxy da tabGroups API
   */
  createTabGroupsProxy() {
    ...
    return new Proxy(browser.tabGroups, {
      get(target, prop) {
        switch (prop) {
          case 'get':
            return (groupId, context = {}) => self.executeWithRateLimit(
          ...
          case 'update':
            return (groupId, updateProperties, context = {}) => self.executeWithRateLimit(
  ...
  /**
   * Cria proxy para windows API
   * @returns {Proxy} Proxy da windows API
   */
  createWindowsProxy() {
    const self = this;
    ...
    return new Proxy(browser.windows, {
      get(target, prop) {
        switch (prop) {
          case 'getAll':
            return (getInfo, context = {}) => self.executeWithRateLimit(
  ...
  /**
   * Cria proxy para storage API
   * @returns {Proxy} Proxy da storage API
   */
  createStorageProxy() {
    const self = this;
  ...
  /**
   * Cria proxy para área específica de storage
   * @param {Object} storageArea - Área de storage (local ou sync)
   * @param {string} areaName - Nome da área
   * @returns {Proxy} Proxy da área de storage
   */
  createStorageAreaProxy(storageArea, areaName) {
    ...
    return new Proxy(storageArea, {
      get(target, prop) {
        switch (prop) {
          case 'get':
            return (keys, context = {}) => self.executeWithRateLimit(
          ...
          case 'set':
            return (items, context = {}) => self.executeWithRateLimit(
  ...
  /**
   * Executa operação com rate limiting ou fallback
   * @param {Function} rateLimitedCall - Chamada com rate limiting
   * @param {Function} nativeCall - Chamada nativa
  ...
  async executeWithRateLimit(rateLimitedCall, nativeCall, operationName) {
    if (!this.rateLimitingEnabled) {
      Logger.debug("BrowserAPIWrapper", `Rate limiting desabilitado, usando API nativa: ${operationName}`);
      return nativeCall();
    }
    
    try {
      const result = await rateLimitedCall();
      Logger.debug("BrowserAPIWrapper", `Rate limited call bem-sucedida: ${operationName}`);
      return result;
    } catch (error) {
      Logger.warn("BrowserAPIWrapper", `Rate limited call falhou para ${operationName}: ${error.message}`);
      
      if (this.fallbackToNative) {
        Logger.debug("BrowserAPIWrapper", `Usando fallback para API nativa: ${operationName}`);
        return nativeCall();
      } else {
        throw error;
      }
...
export function createRateLimitedTabsAPI() {
  return {
    ...
    query: (queryInfo, context = {}) => RateLimitedAPI.tabs.query(queryInfo, context),
    group: (options, context = {}) => RateLimitedAPI.tabs.group(options, context),
    ungroup: (tabIds, context = {}) => RateLimitedAPI.tabs.ungroup(tabIds, context)
  };
}

export function createRateLimitedTabGroupsAPI() {
  return {
    get: (groupId, context = {}) => RateLimitedAPI.tabGroups.get(groupId, context),
    query: (queryInfo, context = {}) => RateLimitedAPI.tabGroups.query(queryInfo, context),
    update: (groupId, updateProperties, context = {}) => RateLimitedAPI.tabGroups.update(groupId, updateProperties, context)
  };
}
...
export function createRateLimitedStorageAPI() {
  return {
    local: {
      get: (keys, context = {}) => RateLimitedAPI.storage.local.get(keys, context),
      set: (items, context = {}) => RateLimitedAPI.storage.local.set(items, context)
    },
    sync: {
      get: (keys, context = {}) => RateLimitedAPI.storage.sync.get(keys, context),
      set: (items, context = {}) => RateLimitedAPI.storage.sync.set(items, context)
    }
  };
}

Logger.debug("BrowserAPIWrapper", "Wrapper de APIs do navegador com rate limiting inicializado.");